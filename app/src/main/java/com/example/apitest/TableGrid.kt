...
open class TableGrid {
    ...
    internal open inner class DataAdapter (private val baseLayOutID : Int) : BaseAdapter() {
        ...
        fun setCellStyle(row : Int, col : Int , style : Bundle?)
        {
            ...
            if(style != null) {
                ...
                cellstyle[Point(row,col)] = instyle
            }
            else{
                val instyle = Bundle()
        ...
        {
            var data = cellstyle[Point(row,col)]
            if(data == null)
            {
                val temp = Bundle()
        ...
        */
        ...
        private fun getChidrenViews(v : View?) : ArrayList<View?>
        {
            ...
            for(_idx in 0..childviewcnt-1)
            {
                ...
                val name = tempview?.accessibilityClassName.toString().toLowerCase(Locale.ROOT)
                    .replace(addon, "")
                when(name)
                {
                    "textview"->{
