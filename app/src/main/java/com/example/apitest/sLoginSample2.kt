...
import com.example.datamngr.API_DEFINE
...
//        m_SM.initManager(this)
        ...
        m_editTextPWD = findViewById(R.id.editTextPWD)
        //m_editTextPWD.setText("")
        m_combobox = findViewById(R.id.combobox1)
        m_buttonLogin = findViewById(R.id.buttonLogin)
        m_buttonCert = findViewById(R.id.buttonCert)
        m_textViewNot = findViewById(R.id.textViewNot)
        ...
        if (manager.getAutoLogin()) {
            manager.setDataString("login_type", "2")
            val strDN = manager.getDataString("dn")
            if (strDN.isNotEmpty()) {
                comboSelect(strDN)
            }
        }
...
//        m_nHandle = manager.setHandler(this, handler as Handler)
    ...
    fun OnButtonLoginClicked(v: View) {
        ...
        if (m_password.isEmpty()) {
            Toast.makeText(this, "공동인증 비밀번호를 입력하세요.", Toast.LENGTH_SHORT).show()
            return
        }

        if (m_dn.isEmpty() || m_dn == "공동인증서를 선택하세요.") {
            Toast.makeText(this, "공동인증서를 선택하세요.", Toast.LENGTH_SHORT).show()
            return
        }

        m_buttonLogin.isEnabled = false
